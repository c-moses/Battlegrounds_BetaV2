class BGLayer1 extends BGBaseLayer // Chernarus Full
{
    ...
    override void CheckCaptureRadius()
    {
        ...
        array<string> playersWithin200m = new array<string>();
        array<string> playersWithin300m = new array<string>();
        ...
        for (int i = 0; i < playersInRadius.Count(); i++)
        {
            ...
            {
                playersWithin200m.Insert(player.GetIdentity().GetName());
                currencyAmount = 50;
            }
            else if (distanceToEvent <= 300.0)
            {
                playersWithin300m.Insert(player.GetIdentity().GetName());
                currencyAmount = 20;
            }
            else
            {
